...
const findAllConfigsPaths = async ({ dirPath }: { dirPath: string }) => {
  ...
  while (true) {
    let maybeConfigGlobs = [
      `${dirPathHere}/(libalibe.|libalibe.*.)(js|ts|yml|yaml|json)`,
    ];
    let maybeConfigPath = (
...
export const getConfig = async ({ dirPath }: { dirPath: string }) => {
  ...
  for (const configPath of configPaths) {
    ...
    configMerged.include = [
      ...new Set([
        ...configMerged.include,
        ...(Array.isArray(configData.include) ? configData.include : []),
      ]),
    ];
    configMerged.exclude = [
      ...new Set([
        ...configMerged.exclude,
        ...(Array.isArray(configData.exclude) ? configData.exclude : []),
      ]),
    ];
  }
  ...
  if (!configMergedValidated.success) {
    throw new Error(
      `Invalid config files: "${stringsToLikeArrayString(configPaths)}"`
    );
  }
...
export const getPackageJsonData = async ({ dirPath }: { dirPath: string }) => {
  let dirPathHere = path.resolve("/", dirPath);
  while (true) {
    let maybePackageJsonGlobs = [`${dirPathHere}/package.json`];
